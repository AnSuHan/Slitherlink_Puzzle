...
class GameStateTriangle extends State<GameSceneTriangle> with WidgetsBindingObserver {
  ...
  void _loadPuzzle() async {
    ...
    if (_verifyOnGenerate && !widget.isContinue) {
  ...
  Future<void> _onNewGame() async {
    ...
    final prefs = ExtractData();
    // Drop answer/clue/continue so pushReplacement regenerates a fresh puzzle.
    for (final suffix in ["", "_clue", "_continue"]) {
